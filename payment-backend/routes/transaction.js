...
router.post('/send', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const sanitizedEmail = toEmail.trim();
    const toUser = await User.findOne({ email: { $regex: new RegExp(`^${sanitizedEmail}$`, 'i') } }).session(session);
    if (!toUser) {
      await session.abortTransaction();
...
router.post('/request', authMiddleware, async (req, res) => {
  ...
  try {
    const fromUser = await User.findOne({ email: { $regex: new RegExp(`^${fromEmail.trim()}$`, 'i') } });
    if (!fromUser) {
...
router.post('/request/:id/decline', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!transaction) return res.status(404).json({ error: 'Request not found.' });
        if (transaction.from.toString() !== req.userId) return res.status(403).json({ error: 'You are not authorized to decline this request.' });
        if (transaction.status !== 'pending') return res.status(400).json({ error: 'This request is no longer pending.' });
...
router.get('/recent', authMiddleware, async (req, res) => {
  try {
    const transactions = await Transaction.find({ $or: [{ from: req.userId }, { to: req.userId }] })
      .sort({ timestamp: -1 })
      .limit(20)
    ...
    const formattedTransactions = transactions
      ...
      .map(tx => {
        ...
        return {
          ...
          status: tx.status,
          otherParty: { firstName: otherParty.firstName, lastName: otherParty.lastName, }
        };
      });
...
router.get('/pending', authMiddleware, async (req, res) => {
  try {
    const pendingRequests = await Transaction.find({ from: req.userId, status: 'pending' })
      .sort({ timestamp: -1 })
